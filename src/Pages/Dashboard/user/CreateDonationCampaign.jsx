...
const CreateDonationCampaign = () => {
  ...
  const formik = useFormik({
    ...
    validationSchema: yup.object({
      petName: yup
        .string()
        .min(3, "pet name have must atleast 3 characters")
        .required(),
      petAge: yup.number().required(),
      maxAmount: yup
        .number()
        .min(2, "Max Donation have must atleast 2 numbers")
        .required(),
      lastDate: yup.date().required(),
      shortdes: yup
        .string()
        .min(10, "short description have must atleast 10 characters")
        .required(),
      longDes: yup
        .string()
        .min(50, "long descripton  have must atleast 50 characters")
        .required(),
    ...
    onSubmit: async (values) => {
      ...
      try {
        ...
        axiosSecure
          .post("/donationCampaigns", campaignData)
          .then((res) => {
            if (res?.status === 201) {
      ...
      <form onSubmit={formik.handleSubmit}>
        ...
        <div className="flex flex-col md:flex-row md:gap-10 items-baseline mb-5">
          <div className="flex-1 w-full">
            <label
              htmlFor="maxAmount"
              className="text-neutral-600 max-md:text-sm font-semibold pl-2"
            >
        ...
        <div className="flex  flex-col md:flex-row md:gap-10  items-center  mb-5">
          <div className="flex-1 w-full ">
            ...
            />
            {formik.touched.shortdes && formik.errors.shortdes && (
              <span className="text-sm text-red-600 pl-5">
                {formik.errors.shortdes}
              </span>
            )}
          </div>
          <div className="flex-1 w-full">
        ...
        <div className="">
          ...
          />
          {formik.touched.longDes && formik.errors.longDes && (
            <span className="text-sm text-red-600 pl-5">
              {formik.errors.longDes}
            </span>
          )}
        </div>
        ...
        <button
          type="submit"
          className="bg-[#279c46] rounded cursor-pointer hover:bg-fuchsia-700  py-2 px-5 w-full text-white font-bold text-lg"
        >
          Create Now
        </button>
      </form>
