...
const AdoptionRequest = () => {
  ...
  const {
    ...
    queryFn: async () => {
      const res = await axiosSecure(
        `/userAddedAdoptionRequests?email=${user?.email}`
      );
      return res.data;
    },
  });
  ...
  return (
    <div>
      <h1 className="pt-10 text-5xl text-[#279c46] text-center font-extrabold">
          ...
          <tbody>
            {donationsReq.map((user, index) => (
              <tr key={user._id}>
                ...
                <td>
                  <button
                    onClick={() => handleAcceptReq(user)}
                    className="bg-[#279c46]  lg:p-2 text-white rounded-md"
                  >
                    {user?.accepted === true ? "Accepted" : "Accept"}
                  </button>
                </td>
                <td>
                  <button
                    onClick={() => handleDeleteUserReq(user)}
