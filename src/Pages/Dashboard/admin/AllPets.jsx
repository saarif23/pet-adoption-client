  ...
  const {
    ...
    queryFn: async () => {
      const res = await axiosSecure.get(
        `/allPets?page=${currentPage}&size=${itemPerPage}`
      );
      return res.data;
    },
  });
  ...
  const handleChangeStatus = (item) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axiosSecure
          .patch(`/allPets/${item._id}`, { adopted: !item?.adopted })
          .then(() => {
    ...
    <div>
      ...
      <div className=" bg-white p-5">
        ...
        <table className="md:table p-2 max-md:text-xs  ">
          {/* head */}
          <thead className="bg-[#279c46] py-2 ">
              ...
              <tr key={index}>
                ...
                <th>
                  <Link to={`/dashboard/updatePet/${item._id}`}>
                    {" "}
                    <button className="bg-amber-300  p-2 text-white rounded-md">
                      <FaRegPenToSquare />
                    </button>
                  </Link>
                </th>
                ...
                <th>
                  <button
                    ...
                    className=" bg-[#279c46] max-md::text-[5px] p-1 md:p-2 text-white rounded-md"
                  >
                    Change Status
                  </button>
                </th>
